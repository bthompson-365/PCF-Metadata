...
export class SelectRelationship implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
		...
		this.schemaName=context.parameters.SchemaName.raw||"";
	...
	private async getSchemaName(){
		if (this.currentValue){
			var a=await this.getSchemaDetail(this.formEntity,this.queryEntity, this.currentValue);
			var result = JSON.parse(a);
			this.schemaName=result.value[0].SchemaName;
			this.notifyOutputChanged();
		}
	}

	private async getSchemaDetail(displayEntity: string, searchEntity: string, searchAttribute:string):Promise<string>{
		var req = new XMLHttpRequest();
		var baseUrl=this.baseUrl;
		
		return new Promise(function (resolve, reject) {
		
			req.open("GET", baseUrl + "/api/data/v9.0/RelationshipDefinitions/Microsoft.Dynamics.CRM.OneToManyRelationshipMetadata?$select=SchemaName&$filter=ReferencingAttribute%20eq%20%27"+ searchAttribute +"%27%20and%20ReferencingEntity%20eq%20%27"+searchEntity +"%27%20and%20ReferencedEntity%20eq%20%27"+displayEntity+"%27", true);
