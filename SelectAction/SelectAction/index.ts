...
export class SelectAction implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
		...
		if (entity!==this.entity && entity!==""){
			if (this.firstRun){
				this.currentValue=context.parameters.Action.raw||"";
				this.firstRun=false;
			}
			else {
				this.currentValue="";
	...
	private async populateComboBox(entity:string) {
		...
		if (entity!==""){
			var a = await this.getActions(entity);
			var result = JSON.parse(a);
			var options: IDropdownOption[]=[];
			// format all the options into a usable record
			for (var i = 0; i < result.value.length; i++) {
				
				var uniqueName = await this.getActionUniqueName(result.value[i]._sdkmessageid_value)
				if (result.value[i].name !== null) {
					var text = result.value[i].name + " (" + uniqueName + ")";
					var option: IDropdownOption = { key: uniqueName, text: text }
					options.push(option);
	...
	private async getActions(entity: string):Promise<string> {
		var req = new XMLHttpRequest();
		var baseUrl=this.baseUrl;
		return new Promise(function (resolve, reject) {
			req.open("GET", baseUrl + "/api/data/v9.1/workflows?$select=name,_sdkmessageid_value&$filter=category%20eq%203%20and%20statuscode%20eq%202%20and%20type%20eq%202%20and%20primaryentity%20eq%20%27"+entity+"%27", true);
	...
	private async getActionUniqueName(sdkmessageId: string):Promise<string> {
		var req = new XMLHttpRequest();
		var baseUrl=this.baseUrl;
		return new Promise(function (resolve, reject) {
			req.open("GET", baseUrl + "/api/data/v9.1/sdkmessages?$select=name&$filter=sdkmessageid%20eq%20"+sdkmessageId, true);
			req.onreadystatechange = function () {
				...
				if (req.status >= 200 && req.status < 300) {
					...
					try {
						...
						resolve(JSON.parse(req.responseText).value[0].name);
					}
					catch (error) {
						throw error;
					}
