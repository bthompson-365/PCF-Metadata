...
export class SelectForm implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
		...
		if (entity!==this.entity && entity!==""){
			if (this.firstRun){
				this.currentValue=context.parameters.Form.raw||"";
				this.firstRun=false;
			}
			else {
				this.currentValue="";
	...
	private async getViews(entity: string):Promise<string> {
		...
		return new Promise(function (resolve, reject) {
			...
			req.open("GET", baseUrl + "/api/data/v9.1/systemforms?$top=50&$select=description,name,type,isdefault&$filter=objecttypecode eq '"+entity+"' and type eq 2", true);
